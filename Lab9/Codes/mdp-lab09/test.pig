...
data_filtered = FILTER testin by ( group is not null and TRIM(group) != '' );
grouped_data = GROUP data_filtered BY group;
output_sum = FOREACH grouped_data GENERATE COUNT(data_filtered) AS counter, group AS grupos;
